...
class BatchProcessRequest(BaseModel):
    ...
    limit: int = Field(default=100, ge=1, le=1000, description="Maximum events to process")
...
class ReprocessFailedRequest(BaseModel):
    ...
    source_id: Optional[str] = Field(default=None, description="Optional filter by source")
    limit: int = Field(default=100, ge=1, le=500, description="Maximum events to reprocess")
...
async def process_batch(request: BatchProcessRequest):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=500,
                detail="Failed to trigger batch normalization"
            )
...
async def normalize_source(request: NormalizeSourceRequest):
    ...
    try:
        ...
        if event_ids is None:
            raise HTTPException(
                status_code=500,
                detail="Failed to trigger source normalization"
            )
...
async def reprocess_failed(request: Optional[ReprocessFailedRequest] = None):
    ...
    try:
        ...
        success = await trigger_reprocess_failed(
            source_id=source_id,
            limit=limit
        )
        ...
        if not success:
            raise HTTPException(
                status_code=500,
                detail="Failed to trigger reprocessing"
            )
