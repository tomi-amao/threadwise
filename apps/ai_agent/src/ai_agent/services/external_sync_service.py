...
class ExternalSyncService:
    ...
    async def get_entity(self, entity_id: str) -> Optional[Dict[str, Any]]:
        """Get an entity by ID."""
        result = await asyncio.to_thread(
            lambda: self.client.table("entities").select("*").eq("id", entity_id).execute()
        )
        return result.data[0] if result.data else None
    ...
    async def list_entities(self) -> List[Dict[str, Any]]:
        """List all entities."""
        result = await asyncio.to_thread(
            lambda: self.client.table("entities").select("*").order("created_at").execute()
        )
    ...
    async def create_external_source(
        ...
        if api_key:
            try:
                ...
                logger.info(f"Stored API key securely in Vault for source {source['id']}")
            except Exception as e:
                logger.error(f"Failed to store API key in Vault: {e}")
                # Fall back to legacy storage (will be migrated later)
                await asyncio.to_thread(
                    lambda: self.client.table("external_sources").update({
        ...
        result = await asyncio.to_thread(
            lambda: self.client.table("external_sources").select("*").eq("id", source["id"]).execute()
        )
