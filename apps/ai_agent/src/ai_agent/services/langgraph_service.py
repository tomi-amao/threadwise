...
class LangGraphService:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.client:
                return {
                    "status": "error",
                    "message": "LangGraph client not initialized",
                }
    ...
    async def list_assistants(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for assistant in assistants:
                assistant_details = await self.client.assistants.get(
                    assistant["assistant_id"]
                )
                assistants_list.append(assistant_details)
            return {"status": "success", "assistants": assistants_list}
        except Exception as e:
            return {
                "status": "error",
    ...
    async def send_message(
        ...
        try:
            ...
            message_input = {"messages": [{"role": "human", "content": content}]}
            ...
            print(
                f"Sending message to thread {thread_id}: {message_input} Assistant ID: {assistant_id}"
            )
            # Create and run the thread
            run = await self.client.runs.create(
                thread_id=thread_id, assistant_id=assistant_id, input=message_input
            )
